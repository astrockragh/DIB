...
done | parallel --will-cite $BAR_FLAG -j $NUM_PROCS --colsep '\t' process_combination {1} {2} {3} {4} {5}

# Process first batch with Julia
echo -e "\nProcessing first batch with Julia..."
julia ingest_pgo_asymtop.jl "$OUTPUT_DIR" "${OUTPUT_DIR}_original"
...
julia ingest_pgo_asymtop.jl "$OUTPUT_DIR" "${OUTPUT_DIR}_epsilon"
