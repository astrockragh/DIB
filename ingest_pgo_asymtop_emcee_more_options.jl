...
function parse_pgo_filename(filename)
    # Example filename: spec_T30.0_A0.001200_B0.001200_C0.001200_FA1.000_FB1.000_FC1.000.txt
    parts = split(filename, "_")
    temp_str        = replace(parts[3], "T" => "")
    a_str           = replace(parts[4], "A" => "")
    b_str           = replace(parts[5], "B" => "")
    c_str           = replace(parts[6], "C" => "")
    fraction_a_str  = replace(parts[7], "FA" => "")
    fraction_b_str  = replace(parts[8], "FB" => "")
    fraction_c_str  = replace(replace(parts[9], "FC" => ""), ".txt" => "")
    return (
        parse(Float64, temp_str),
        parse(Float64, a_str),
        parse(Float64, b_str),
        parse(Float64, c_str),
        parse(Float64, fraction_a_str),
        parse(Float64, fraction_b_str),
...
function process_pgo_outputs(input_file, savename)
...
if length(ARGS) != 2
    error("Usage: julia ingest_pgo_asym.jl <output_file> <savename>")
end
