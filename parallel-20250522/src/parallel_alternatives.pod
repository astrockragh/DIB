...
B<ppss> is a tool for running jobs in parallel.
...
=head2 DIFFERENCES BETWEEN pararun/paramap AND GNU Parallel
...
=item - O2 O3 (O4) O5 O6 - - x -

=item E1 -  (E3) E4 (E5) - - E8 E9
...
B<pararun> gets tricky when you need to quote more than 2 levels:

  $ echo "$bmps" | parallel -q perl -e 'print @ARGV,"\n"' {}

  $ echo "$bmps" | pararun 'perl -e '"'"'print @ARGV,"\n"'"'"' "$1"'

B<paramap> tries to keep order of output, and works for short lines
on stdout:
...
B<paramap> does not deal with stderr:
...
B<paramap> only prints, when all jobs are done. GNU B<parallel> prints
as soon as possible.

Fails B<paramap> to run (due to final ;):
...
(Last checked: 2025-01)


=head2 DIFFERENCES BETWEEN concur AND GNU Parallel
...
B<concur> outputs in json. Output is cached in RAM and takes up a
factor of 6: 1 GB output uses 6 GB RAM.
...
It gives the wrong output on jobs like:
  ...
  concur 'bash -c "sleep {{1}}; touch a{{1}}"' 3 2 1

=head3 EXAMPLES

  1$ concur "dig @{{1}} ocw.mit.edu" 1.1.1.1 9.9.9.9 8.8.8.8 94.140.14.14 \
     208.67.222.222

  1$ parallel --results -.json  "dig @{1} ocw.mit.edu" 1 ::: 1.1.1.1 9.9.9.9\
     8.8.8.8 94.140.14.14 208.67.222.222 | jq .

  2$ echo "foo bar baz" | concur "touch /tmp/{{1}}.foo"

  2$ printf "%s\n" foo bar baz | parallel "touch /tmp/{1}.foo"

  3$ concur "ping -c 1 {{1}}" www.mit.edu www.ucla.edu www.slashdot.org

  3$ parallel --results -.json "ping -c 1 {1}" ::: www.mit.edu \
     www.ucla.edu www.slashdot.org | jq .

  4$ concur "ping -c 1 {{1}}" www.mit.edu www.ucla.edu www.slashdot.org |
     jq '.command[0]'

  4$ parallel --results -.json "ping -c 1 {1}" ::: www.mit.edu www.ucla.edu \
