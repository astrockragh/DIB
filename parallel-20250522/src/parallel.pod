...
I<n>'th argument (when used with B<-N> or B<--colsep>).

If I<n> is negative it refers to the I<n>'th last argument.
...
but the columns from each input source will be linked. Here {4} refers
to column 2 in input source 2:
...
See also: B<--resume> B<--resume-failed> B<--progress>
...
If I<num> ends in auto, GNU B<parallel> will use the value as a max
value. If a job fails, GNU B<parallel> will adjust the number of jobs
down with a factor. If a job succeeds GNU B<parallel> will adjust the
number of jobs up with a small factor.

So if the number of jobs is a bit too high, GNU B<parallel> will
dynamically lower it.
...
This will start 20 jobs in parallel. These will fail because more than 5
sleeps are running. GNU B<parallel> will adjust the number of jobs
until around 5 are running in parallel.
...
Print the latest line. Each job gets a single line on the screen that
is updated with the last full line from currently running jobs.

The screen keeps the oldest running job in view, so younger jobs may
no be visible. These will, however, be shown when the oldest job
finishes.

Running this example makes it easer to understand. Note how the screen
scrolls when the job at the top of the screen finishes:
...
See also: UNIT PREFIX B<--term-seq> B<--memsuspend>
...
This will run (a, b, c), (1, 2), (A, B), (I, II, III). The jobs in a
batch will be run in parallel, but the batches will be run in serial.

Note how the start time for each batch is later than the end time of
the previous batch.
...
This will complete First_run with all files before starting Second_run
on all files.
...
This runs: [a] x [A B C], [a] x [I II], [b] x [A B C], [b] x [I II],
[1] x [A B C], [1] x [I II], [2] x [A B C], [2] x [I II], [3] x [A B
C], [3] x [I II] = (a A, a B, a C), (a I, a II), (b A, b B, b C), (b
I, b II), (1 A, 1 B, 1 C), (1 I, 1 II), (2 A, 2 B, 2 C), (2 I, 2 II),
(3 A, 3 B, 3 C), (3 I, 3 II).

The jobs in a batch will be run in parallel, but the batches will be
run in serial.

When the bug is fixed, it will run: [a b] x [A B C], [a b] x [I II],
[1 2 3] x [A B C], [1 2 3] x [I II] = (a A, a B, a C, b A, b B, b C),
(a I, a II, b I, b II), (1 A, 1 B, 1 C, 2 A, 2 B, 2 C, 3 A, 3 B, 3 C),
...
number of CPU threads on the host. Normally I<ncpus> will not be
needed.

Examples: B<4/>, B<12/>


=item Z<> I<ssh command>
...
Ranges of hostnames can be given in [] like this: B<server[1,3,8-10]>
(for B<server1, server3, server8, server9, server10>) or
B<server[001,003,008-010]> (for B<server001, server003, server008,
server009, server010>). With Bash's brace expansion you can do:
B<-S{dev,prod}[001-100]> to get B<-Sdev[001-100] -Sprod[001-100]> More
[]'s are allowed: B<server[01-10].cluster[1-5].example.net>

=item Z<> :I<port>

Port number to connect to.

Examples: B<:22>, B<:2222>.

For IPv6 you can use B<p> or B<#> instead of B<:>.

Examples: B<[2001:470:142:4::a]:2222>, B<2001:470:142:5::116p2222>,
B<2001:470:142:5::116#22222>
...
Read sshlogins from B<~/.parallel/sshloginfile> or
B<$XDG_CONFIG_HOME/parallel/sshloginfile>
...
See B<--sshloginfile> for complete examples.
...
Read sshlogins from B<~/.parallel/sshloginfile>

=item Z<> I<.>

Read sshlogins from B</etc/parallel/sshloginfile>
...
GNU B<parallel> tries to be conservative to avoid surprising
results. B<--unsafe> will allow GNU B<parallel> to run in environments
and on input that are untested and thus may cause surprising results
and even security issues, where an evil attacker can influence the
results. Think attacks similar to Shellshock:
https://en.wikipedia.org/wiki/Shellshock_(software_bug)

If you are forced to use B<--unsafe> all the time for something that
is safe, it is time to file a bug report and have a discussion how to
make your situation well tested.
